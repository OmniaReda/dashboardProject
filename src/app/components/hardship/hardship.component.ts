...
export class HardshipComponent implements OnInit {
  ...
  getDonughtData() {
    this.http
      .get(
        'https://quilled-autumn-move.glitch.me/api-gateway-odoo/api/Dashboard/HardshipTypes'
      )
      .pipe(
  ...
  getBarData() {
    ...
    this.http
      .get(
        'https://quilled-autumn-move.glitch.me/api-gateway-odoo/api/Dashboard/HardshipTypesTime'
      )
      .pipe(
        catchError((error: HttpErrorResponse) => {
          console.error('Error fetching dashboard data:', error);
      ...
      .subscribe((res: any) => {
        ...
        this.progressBars = this.barData.Labels.map(
          (label: string, index: number) => {
            let percentageP1: number = Math.abs(
              this.barData.DataReports[0].Data[index] /
                this.barData.DataReports[0].Data[index] +
                this.barData.DataReports[1].Data[index] * 100
            );
            ...
            let progressbar = {
              ...
              count:
                this.barData.DataReports[0].Data[index] +
                this.barData.DataReports[1].Data[index],
              p1: {
                color: '#c00000',
                count: this.barData.DataReports[0].Data[index],
                percentage: percentageP1,
              },
              p2: {
                color: '#bd9f57',
                count: this.barData.DataReports[1].Data[index],
